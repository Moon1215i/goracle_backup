...
csv_file="https://github.com/Moon1215i/goracle_backup/translations/translations.csv"
...
if [ $sshkey_exists -eq 1 ]; then
    ...
    # ローカルディレクトリがなければ作成
    echo -e "${YELLOW}mkdir -p $local_dir && rsync -avz --progress -e 'ssh -i ${NC}${RED}${private_key}${NC}${YELLOW} -p ${ssh_port}' ${remote_user}@${ip_address}:$backup_file ${local_dir}${NC}"
    echo
    echo -e "${YELLOW}-----------------------------------------------------------------------------------------------------${NC}"
    ...
    echo -e "${NC}-----------------------------------------------------------------------------------------------------${NC}"
    echo
    echo -e "${NC}rsync -avz --progress -e 'ssh -i ${RED}${private_key}${NC} -p ${ssh_port}' ${local_dir}${backup_file} ${remote_user}@${ip_address}:$backup_file${NC}"
    echo
    echo -e "${NC}-----------------------------------------------------------------------------------------------------${NC}"
    ...
    echo -e "${YELLOW}-----------------------------------------------------------------------------------------------------${NC}"
    echo
    echo -e "${YELLOW}mkdir -p $local_dir && rsync -avz --progress -e 'ssh -p ${ssh_port}' ${remote_user}@${ip_address}:$backup_file ${local_dir}${NC}"
    echo
    echo -e "${YELLOW}-----------------------------------------------------------------------------------------------------${NC}"
    echo -e "${YELLOW}$DOWNLOAD_BACKUP${NC}"
    echo
    ...
    echo -e "${NC}-----------------------------------------------------------------------------------------------------${NC}"
    echo
    echo -e "${NC}rsync -avz --progress -e 'ssh -p ${ssh_port}' ${local_dir}${backup_file} ${remote_user}@${ip_address}:$backup_file${NC}"
    echo
    echo -e "${NC}-----------------------------------------------------------------------------------------------------${NC}"
    echo -e "${NC}$UPLOAD_BACKUP${NC}"
    echo
